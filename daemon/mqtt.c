...
static void mqtt_ssrc_stats(struct ssrc_ctx *ssrc, JsonBuilder *json, struct call_media *media) {
	...
	json_builder_add_int_value(json, atomic64_get(&ssrc->packets));

	json_builder_set_member_name(json, "bytes");
	json_builder_add_int_value(json, atomic64_get(&ssrc->octets));

	json_builder_set_member_name(json, "lost");
	json_builder_add_int_value(json, atomic64_get(&ssrc->packets_lost));

	json_builder_set_member_name(json, "duplicates");
	json_builder_add_int_value(json, atomic64_get(&ssrc->duplicates));
...
static void mqtt_global_stats(JsonBuilder *json) {
	...
	json_builder_set_member_name(json, "packets");
	json_builder_add_int_value(json, atomic64_get(&rtpe_stats_cumulative.packets));

	json_builder_set_member_name(json, "bytes");
	json_builder_add_int_value(json, atomic64_get(&rtpe_stats_cumulative.bytes));

	json_builder_set_member_name(json, "errors");
	json_builder_add_int_value(json, atomic64_get(&rtpe_stats_cumulative.errors));
