...
static void __make_passthrough(struct codec_handler *handler, int dtmf_pt, int cn_pt) {
	...
	ilog(LOG_DEBUG, "XXXXXXXXX passthrough handler %p", handler);
...
static void __make_transcoder(struct codec_handler *handler, struct rtp_payload_type *dest,
		...
		int cn_payload_type)
{
	ilog(LOG_DEBUG, "XXXXXXXXX transcode handler %p", handler);
		...
		const struct sdp_ng_flags *flags, const struct stream_params *sp)
{
	...
	for (GList *l = receiver->codecs.codec_prefs.head; l; ) {
		...
		ilog(LOG_DEBUG, "YYYYYYYYYYYYYYYYYYYYY %p %p %p %p", sink_dtmf_pt, sink_cn_pt,
				recv_dtmf_pt, recv_cn_pt);
...
static int __handler_func_sequencer(struct media_packet *mp, struct transcode_packet *packet)
{
	...
	ilog(LOG_DEBUG, "XXXXXXXXXXXX sequencer %p", h);

	if (G_UNLIKELY(!h->ssrc_hash)) {
		ilog(LOG_DEBUG, "XXXXXXXXXXXX no hash");
		if (!packet->func || !h->input_handler->ssrc_hash) {
			ilog(LOG_DEBUG, "XXXXXXXXXXXX no p func");
			h->func(h, mp);
	...
	ilog(LOG_DEBUG, "XXXXXXXXXXXX ch %p", ch);
	if (G_UNLIKELY(!ch)) {
		__transcode_packet_free(packet);
		return 0;
	}
	...
	ilog(LOG_DEBUG, "XXXXXXXXXXXX input ch %p", input_ch);
			...
			struct transcode_packet *, struct media_packet *))
{
	...
	ilog(LOG_DEBUG, "XXXXXXXXXX input handler %p passthrough %u", sequencer_h, sequencer_h->passthrough);
...
void codec_tracker_update(struct codec_store *cs) {
	...
	for (GList *l = supp_codecs; l; l = l->next) {
		...
		while (to_remove) {
			...
			ilog(LOG_DEBUG, "XXXXXXXXXXXXXX %u %u %p %p", clockrate, sct->all_touched, cs, cs->tracker);
			...
			ilog(LOG_DEBUG, "XXXXXXXXXXXX entries %p %i", entries, entries->length);
